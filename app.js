...
async function initializeData() {
    ...
    onValue(listsRef, (snapshot) => {
        ...
        if (data) {
            ...
            if (!state.activeListId && state.lists.length > 0) state.activeListId = state.lists[0].id;
    ...
    sortedItems.forEach((item) => {
        ...
        li.innerHTML = `
            <div class="flex items-center justify-center">
                <div class="w-5 h-5 md:w-4 md:h-4 rounded-full border flex items-center justify-center cursor-pointer transition-all ${item.done ? 'bg-zinc-900 border-zinc-900 shadow-inner' : 'border-zinc-300 bg-white hover:border-zinc-600'}" onclick="toggleItem('${item.id}', event)">
            ...
            try {
                const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${model}:generateContent?key=${state.apiKey}`, {
                    ...
                    body: JSON.stringify({ contents: [{ parts: [
                        { text: "Analyze this image and identify all items that could be part of a shopping list. Return ONLY a valid JSON array of objects with 'title', 'qty', and 'brand' keys. Use Turkish for titles. No markdown." },
...
function addItem(title, qty, brand) {
    ...
    push(itemsRef).then(newRef => set(newRef, { title, qty, brand, done: false }));
}
